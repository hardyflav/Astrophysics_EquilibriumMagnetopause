...
Plots = ["CurveEquator", "CurveMeridianParts", "CurveMeridian", "SurfaceInitialTot"];
[SurfaceInitialTot, ThetaCusp, rEquator, rMeridian, rSubSolarNose] = InitialGuess(SystemParameters, ThetaMaxDeg, PhiMaxDeg, DeltaThetaDeg, DeltaPhiDeg, Plots);



%%% [ TEST - RESCALING INITIAL CAN DISK SURFACE

CrossSectionsZ = rMeridian.'.*cos(ThetaSpan.');
YMeridian = rMeridian.'.*sin(ThetaSpan.');

ZMeridianCAN = rMeridianCAN./rMeridianCAN(1).*cos(ThetaSpan.');
YMeridianCAN = rMeridianCAN.*sin(ThetaSpan.');

T1 = griddedInterpolant(-ZMeridianCAN, YMeridianCAN);
CrossSectionsYCAN = T1(-CrossSectionsZ);
FactorMeridian = CrossSectionsYCAN(2:end) ./ (YMeridian(2:end));



ZEquatorCAN = rEquatorCAN./rEquatorCAN(1).*cos(ThetaSpan.');
YEquatorCAN = rEquatorCAN.*sin(ThetaSpan.');

T2 = griddedInterpolant(-ZEquatorCAN, YEquatorCAN);
CrossSectionsYCAN2 = T2(-CrossSectionsZ);
FactorEquator = CrossSectionsYCAN2(2:end) ./ (YMeridian(2:end));
...
PlotsCorrection = ["GridWrappedAbsolute"];
