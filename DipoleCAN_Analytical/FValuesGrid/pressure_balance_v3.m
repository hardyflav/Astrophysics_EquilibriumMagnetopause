function [pressure_balance_v3] = pressure_balance_v3( k, r, rss, r_eq, r_mer, delta_theta_deg, delta_phi_deg, Max_deg_equ, Max_deg_phi_direction, beta, CAN_DiskParameters )
    ...
    BDisk = B_CAN_Corrected(CAN_DiskParameters, r(k), theta_vec(k), phi_vec(k)).';
    Btot = B+BDisk;
    
    PressureDifference = norm(cross(n_vec, Btot))*sqrt(1+beta) + (1/2)*dot(n_vec,v);
    MeanPressure = (1/2) * (norm(cross(n_vec, Btot))*sqrt(1+beta) - (1/2)*dot(n_vec,v));
    pressure_balance_v3 = PressureDifference / MeanPressure;
