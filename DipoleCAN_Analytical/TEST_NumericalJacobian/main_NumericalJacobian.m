...
%% Grid Definition for Curves and Boundaries
ThetaMaxDeg = 120;                             % Maximum value of theta on the grid, degrees
PhiMaxDeg = 90;                                % Maximum value of phi on the grid, degrees
DeltaThetaDegCurves = 1/2;
DeltaPhiDegCuves = 1/2;
ThetaSpanCurves = (0 : DeltaThetaDegCurves : (ThetaMaxDeg))*pi/180;
PhiSpanCurves = (0 : DeltaPhiDegCuves: PhiMaxDeg).*pi/180;
[NbPointsGridCurves, NbPointsGridInsideCurves,      ...
NbPointsThetaInsideCurves, NbPointsPhiInsideCurves, ...
ThetaSpanVectorCurves, PhiSpanVectorCurves]         = GridDetails(ThetaMaxDeg, PhiMaxDeg, DeltaThetaDegCurves, DeltaPhiDegCuves);
...
% Plots = ["CurveEquator", "CurveMeridian", "SurfaceInitialTot", "GridWrappedAbsolute"];
Plots = ["CurveEquator", "CurveMeridianParts", "CurveMeridian"];
[SurfaceInitialTot, ThetaCuspCurves, rEquatorCurve, rMeridianCurve, rSubSolarNose] = InitialGuess(SystemParameters, ThetaMaxDeg, PhiMaxDeg, DeltaThetaDegCurves, DeltaPhiDegCuves, Plots);

InterpolantEquator = griddedInterpolant(ThetaSpanCurves, rEquatorCurve);
InterpolantMeridian = griddedInterpolant(ThetaSpanCurves, rMeridianCurve);

[PhiSpanInterpolant, ThetaSpanInterpolant] = ndgrid(PhiSpanCurves, ThetaSpanCurves);
InterpolantSurface = griddedInterpolant(PhiSpanInterpolant, ThetaSpanInterpolant, SurfaceInitialTot);
...
rEquator = InterpolantEquator(ThetaSpan).';
rMeridian = InterpolantMeridian(ThetaSpan).';

[PhiSpanOptimisation, ThetaSpanOptimisation] = ndgrid(PhiSpan, ThetaSpan);
SurfaceInitialTot = InterpolantSurface(PhiSpanOptimisation, ThetaSpanOptimisation);
