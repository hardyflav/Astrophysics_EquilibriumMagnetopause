...
function PhiRmpPsw_Interp = PhiRmpPsw_Interp( Conditions_PhiTilt_rMP_Psw )
    ...
    if isnan(Conditions_PhiTilt_rMP_Psw(3))
        ...
        Phi = Conditions_PhiTilt_rMP_Psw(1);
        logRmp = log10(Conditions_PhiTilt_rMP_Psw(2));
    ...
    elseif isnan(Conditions_PhiTilt_rMP_Psw(2))
        ...
        Phi = Conditions_PhiTilt_rMP_Psw(1);
        logPsw = log10( Conditions_PhiTilt_rMP_Psw(3) );
        
        Cont = contourc( PhiList, log10(rMPList), log10(PswValues), [logPsw logPsw]);
        logrMPInterp = griddedInterpolant( Cont(1,2:end), Cont(2,2:end), 'spline' );
