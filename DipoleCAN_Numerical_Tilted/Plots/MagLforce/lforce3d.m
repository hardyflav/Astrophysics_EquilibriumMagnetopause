function Plot=lforce3d(n, Extent, clr, ParaSystem, Rotation, Colour)
...
% Draw Line of Force
% h = figure;
Plot = plot3(x,y,z, 'Color', Colour);                 %,'LineWidth',1.0);
Tilt = ParaSystem.Tilt;
...
if strcmp(Rotation, 'DipoleTilted')
    for i = 1:length(Plot)
        ...
        rotate( Plot(i) , [0, 1, 0], Tilt.Theta  * 180/pi)
        rotate( Plot(i) , [0, 0, 1], Tilt.Phi * 180/pi)
    end
else
    for i = 1:length(Plot)
        Plot(i).Color(4) = 0.2;
...
% 
% if strcmp(Rotation, 'DipoleTilted')
%         rotate( Sphere , [0, 1, 0], Tilt.Theta  * 180/pi)
%         rotate( Sphere , [0, 0, 1], Tilt.Phi * 180/pi)
% end
