...
%     pcolor(ErrorAbsolute(D:C:end, D:C:end)); 

% The 'problematic' contour is eliminated by considering the thetavalues
% (ThetaSpan(1), ThetaSpan(2:2:end)) -> in the theta direction, the
% resolution is 1 deg.
    ErrorAbsoluteCorrection = horzcat( ErrorAbsolute(:,1),  ErrorAbsolute(:,2:2:end));
    pcolor(ErrorAbsoluteCorrection);
    shading flat;
    ...
    ImgRGB = imread('ColourMap.png');
    [ImRows, ImCols, ImPlanes] = size(ImgRGB);
%     ConstructionSurfaceWarp(SurfaceCorrected(D:C:end, D:C:end).',ThetaSpanConcatenated(D:C:end), PhiSpanConcatenated(D:C:end), ImgRGB)
    ConstructionSurfaceWarp( horzcat(SurfaceCorrected(:,1), SurfaceCorrected(:,2:2:end)).', horzcat(ThetaSpanConcatenated(:, 1), ThetaSpanConcatenated(:, 2:2:end)), PhiSpanConcatenated, ImgRGB);
    axis([-1.7 1.7 -1.7 1.7 -1.7 1.7])
