function [pressure_balance_v3, ScalarProduct] = pressure_balance_v3_Bottom( k, r, rLeft, r_eq, r_right_outer_boundary, r_mer, delta_theta_deg, delta_phi_deg, ThetaLeftBoundaryDeg, Max_deg_equ, Max_deg_phi_direction )
    ...
    % Outer boundary 'facing' interior right boundary; theta = theta_max,
    % phi = delta_phi : delta_phi : max_phi - delta_phi
    r_right = r_right_outer_boundary(2:end-1);
%     r_mer = r_mer_vec(Max_deg_equ, delta_theta_deg);
    rLeftBoundary = rLeft(2:end-1);
    
%drdtheta = @(k) (  cos(theta_vec(k))*sin(theta_vec(k))*r(k)^7 - r(k)*sqrt( -1 + r(k)^6 )   ) / (    1 - r(k)^6*sin(theta_vec(k))^2    );
    
    if k == 1
        
        n_vec = [ 1, -1/r(k)*(r(k+N_phi)-rLeftBoundary(k)) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r(k+1)-r_eq(2)) / (2*delta_phi)];
    ...
    elseif k == N_phi
       ...
       n_vec = [ 1, -1/r(k)*(r(k+N_phi)-rLeft(end)) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_mer(2)-r(k-1)) / (2*delta_phi)];
%        n_vec = [ 1, -1/r(k)*(r(k+N_phi)-rss) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_eq(2)-r(k-1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];
        
    elseif k == N_phi*N_theta-N_phi+1

        n_vec = [ 1, -1/r(k)*(r_right(1) -r(k-N_phi))/ (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r(k+1)-r_eq(N_theta+1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];        
        
    elseif k == N_phi*N_theta
        
%          n_vec = [ 1, -1/r(k)*(r_right(end)-r(k-N_phi))/ (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_mer(end-1)-r(k-1)) / (2*delta_phi)];
        n_vec = [ 1, -1/r(k)*(r_right(end)-r(k-N_phi))/ (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_mer(N_theta+1)-r(k-1)) / (2*delta_phi)];
        %        n_vec = [ 1, -1/r(k)*(r_right(end)-r(k-N_phi))/ (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_eq(N_theta+1)-r(k-1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];       
        
    elseif k>1 && k<N_phi   %%% left boundary

        n_vec = [ 1, -1/r(k)*(r(k+N_phi)-rLeftBoundary(k)) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r(k+1)-r(k-1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];
...
%        n_vec = [ 1, -1/r(k)*(r(k+N_phi)-r(k-N_phi)) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r_eq(k/N_phi+1)-r(k-1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];        
        
    elseif k>N_phi*N_theta-N_phi+1 && k<N_phi*N_theta  %%% right boundary

        n_vec = [ 1, -1/r(k)*(r_right( k-(N_phi*N_theta-N_phi) )-r(k-N_phi)) / (2*delta_theta), -1/(r(k)*sin(theta_vec(k))) * (r(k+1)-r(k-1)) / (2*delta_phi)];
        B = M*(1/r(k))^3 * [ -2*sin(theta_vec(k))*sin(phi_vec(k)), cos(theta_vec(k))*sin(phi_vec(k)), cos(phi_vec(k)) ];
        v = [ -cos(theta_vec(k)), sin(theta_vec(k)), 0 ];
